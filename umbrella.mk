...
.PRECIOUS:  umbrella-rot-*.png
.SECONDARY: umbrella-rot-*.png
...
LOGOEXT=png
LOGO=umbrella-logo.$(LOGOEXT)
LOGO_VISIBLE=umbrella-logo-visible.$(LOGOEXT)
LOGO_MIDVISIBLE=umbrella-logo-midvisible.$(LOGOEXT)
ANIMEXT=gif
LOGO_ANIM=umbrella-logo-animated.$(ANIMEXT)
LOGO_ANIM_SMALL=umbrella-logo-small-animated.$(ANIMEXT)
...
test:
	# extract frames
	convert -coalesce umbrella-logo-stego-animated.$(ANIMEXT) umbrella-logo-stego-rot-%02d.$(LOGOEXT)
	# de-stego
	for k in `seq -w 0 1 $$(($(NROT) - 1))` ; do \
	  stegosuite -x -f archive.tlrzpq.gpg.part$(D) umbrella-logo-stego-rot-%02d.$(LOGOEXT) || exit 2 ; \
	done
	# unsplit
...
#$(LOGO): kali.$(LOGOEXT) umbrella-resize.$(LOGOEXT)
$(LOGO): umbrella-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(INVISIBLE) $(SHELL) -c '$(GENLOGO)'
#$(LOGO_VISIBLE): kali.$(LOGOEXT) umbrella-resize.$(LOGOEXT)
$(LOGO_VISIBLE): umbrella-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(VISIBLE) $(SHELL) -c '$(GENLOGO)'
$(LOGO_MIDVISIBLE): umbrella-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(MIDVISIBLE) $(SHELL) -c '$(GENLOGO)'

umbrella-logo.txt: umbrella-logo-visible.$(LOGOEXT)
	img2txt $^ > $@

kali-resize.$(LOGOEXT): kali.$(LOGOEXT) umbrella.dim
	$(RESIZE) -resize `cat umbrella.dim`^ -extent `cat umbrella.dim` $< $@

umbrella.dim:
	echo 2000x2000 > $@

#umbrella-resize.$(LOGOEXT): umbrella-transparent.$(LOGOEXT) umbrella.dim
umbrella-resize.$(LOGOEXT): umbrella-2.$(LOGOEXT) umbrella.dim
	$(RESIZE) -resize `cat umbrella.dim`^ -extent `cat umbrella.dim` $< $@

umbrella-transparent.$(LOGOEXT): umbrella.$(LOGOEXT)
	$(CONVERT) $^ $(TRANSPARENT) $@
umbrella-2.$(LOGOEXT): umbrella.$(LOGOEXT)
...
kali.jpg: kali.url
	$(WGET)
umbrella.png: umbrella.url
	$(WGET)



distclean: cleaner
	$(RM) umbrella.jpg kali.jpg
...
cleaner: clean
	$(RM) $(LOGO) $(LOGO_VISIBLE) $(LOGO_MIDVISIBLE) \
	      ...
	      doxygen-umbrella-logo.$(LOGOEXT)                    \
              gpg-umbrella-logo.jpg avatar.$(LOGOEXT)             \
	      sphinx-umbrella-logo.$(LOGOEXT)                     \
	      stackoverflow-umbrella-logo.$(LOGOEXT)              \
	      small-thumbnail.$(LOGOEXT)                 \
	      large-thumbnail.$(LOGOEXT)                 \
	      wallpaper*.$(LOGOEXT) stripe.jpg           \
	      stripe-icon.$(LOGOEXT) umbrella-logo.txt            \
	      google-cover-umbrella-logo.$(LOGOEXT)               \
	      youtube-watermark-umbrella-logo.$(LOGOEXT)          \
...
clean:
	$(RM) *.dim kali*.$(LOGOEXT) umbrella*.$(LOGOEXT)         \
	      umbrella-logo-rot-*.$(LOGOEXT) umbrella-logo-animated-*.$(LOGOEXT) \
