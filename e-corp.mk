...
.PRECIOUS:  e-corp-rot-*.png
.SECONDARY: e-corp-rot-*.png
...
LOGOEXT=png
LOGO=e-corp-logo.$(LOGOEXT)
LOGO_VISIBLE=e-corp-logo-visible.$(LOGOEXT)
LOGO_MIDVISIBLE=e-corp-logo-midvisible.$(LOGOEXT)
ANIMEXT=gif
LOGO_ANIM=e-corp-logo-animated.$(ANIMEXT)
LOGO_ANIM_SMALL=e-corp-logo-small-animated.$(ANIMEXT)
...
test:
	# extract frames
	convert -coalesce e-corp-logo-stego-animated.$(ANIMEXT) e-corp-logo-stego-rot-%02d.$(LOGOEXT)
	# de-stego
	for k in `seq -w 0 1 $$(($(NROT) - 1))` ; do \
	  stegosuite -x -f archive.tlrzpq.gpg.part$(D) e-corp-logo-stego-rot-%02d.$(LOGOEXT) || exit 2 ; \
	done
	# unsplit
...
#$(LOGO): kali.$(LOGOEXT) e-corp-resize.$(LOGOEXT)
$(LOGO): e-corp-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(INVISIBLE) $(SHELL) -c '$(GENLOGO)'
#$(LOGO_VISIBLE): kali.$(LOGOEXT) e-corp-resize.$(LOGOEXT)
$(LOGO_VISIBLE): e-corp-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(VISIBLE) $(SHELL) -c '$(GENLOGO)'
$(LOGO_MIDVISIBLE): e-corp-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(MIDVISIBLE) $(SHELL) -c '$(GENLOGO)'

e-corp-logo.txt: e-corp-logo-visible.$(LOGOEXT)
	img2txt $^ > $@

kali-resize.$(LOGOEXT): kali.$(LOGOEXT) e-corp.dim
	$(RESIZE) -resize `cat e-corp.dim`^ -extent `cat e-corp.dim` $< $@

e-corp.dim:
	echo 2000x2000 > $@

#e-corp-resize.$(LOGOEXT): e-corp-transparent.$(LOGOEXT) e-corp.dim
e-corp-resize.$(LOGOEXT): e-corp-2.$(LOGOEXT) e-corp.dim
	$(RESIZE) -resize `cat e-corp.dim`^ -extent `cat e-corp.dim` $< $@

e-corp-transparent.$(LOGOEXT): e-corp.$(LOGOEXT)
	$(CONVERT) $^ $(TRANSPARENT) $@
e-corp-2.$(LOGOEXT): e-corp.$(LOGOEXT)
...
kali.jpg: kali.url
	$(WGET)
e-corp.jpg: e-corp.url
	$(WGET)



distclean: cleaner
	$(RM) e-corp.jpg kali.jpg
...
cleaner: clean
	$(RM) $(LOGO) $(LOGO_VISIBLE) $(LOGO_MIDVISIBLE) \
	      ...
	      doxygen-e-corp-logo.$(LOGOEXT)                    \
              gpg-e-corp-logo.jpg avatar.$(LOGOEXT)             \
	      sphinx-e-corp-logo.$(LOGOEXT)                     \
	      stackoverflow-e-corp-logo.$(LOGOEXT)              \
	      small-thumbnail.$(LOGOEXT)                 \
	      large-thumbnail.$(LOGOEXT)                 \
	      wallpaper*.$(LOGOEXT) stripe.jpg           \
	      stripe-icon.$(LOGOEXT) e-corp-logo.txt            \
	      google-cover-e-corp-logo.$(LOGOEXT)               \
	      youtube-watermark-e-corp-logo.$(LOGOEXT)          \
...
clean:
	$(RM) *.dim kali*.$(LOGOEXT) e-corp*.$(LOGOEXT)         \
	      e-corp-logo-rot-*.$(LOGOEXT) e-corp-logo-animated-*.$(LOGOEXT) \
