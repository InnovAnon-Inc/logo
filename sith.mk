...
.PRECIOUS:  sith-rot-*.png
.SECONDARY: sith-rot-*.png
...
LOGOEXT=png
LOGO=sith-logo.$(LOGOEXT)
LOGO_VISIBLE=sith-logo-visible.$(LOGOEXT)
LOGO_MIDVISIBLE=sith-logo-midvisible.$(LOGOEXT)
ANIMEXT=gif
LOGO_ANIM=sith-logo-animated.$(ANIMEXT)
LOGO_ANIM_SMALL=sith-logo-small-animated.$(ANIMEXT)
...
test:
	# extract frames
	convert -coalesce sith-logo-stego-animated.$(ANIMEXT) sith-logo-stego-rot-%02d.$(LOGOEXT)
	# de-stego
	for k in `seq -w 0 1 $$(($(NROT) - 1))` ; do \
	  stegosuite -x -f archive.tlrzpq.gpg.part$(D) sith-logo-stego-rot-%02d.$(LOGOEXT) || exit 2 ; \
	done
	# unsplit
...
#$(LOGO): kali.$(LOGOEXT) sith-resize.$(LOGOEXT)
$(LOGO): sith-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(INVISIBLE) $(SHELL) -c '$(GENLOGO)'
#$(LOGO_VISIBLE): kali.$(LOGOEXT) sith-resize.$(LOGOEXT)
$(LOGO_VISIBLE): sith-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(VISIBLE) $(SHELL) -c '$(GENLOGO)'
$(LOGO_MIDVISIBLE): sith-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(MIDVISIBLE) $(SHELL) -c '$(GENLOGO)'

sith-logo.txt: sith-logo-visible.$(LOGOEXT)
	img2txt $^ > $@

kali-resize.$(LOGOEXT): kali.$(LOGOEXT) sith.dim
	$(RESIZE) -resize `cat sith.dim`^ -extent `cat sith.dim` $< $@

sith.dim:
	echo 2000x2000 > $@

#sith-resize.$(LOGOEXT): sith-transparent.$(LOGOEXT) sith.dim
sith-resize.$(LOGOEXT): sith-2.$(LOGOEXT) sith.dim
	$(RESIZE) -resize `cat sith.dim`^ -extent `cat sith.dim` $< $@

sith-transparent.$(LOGOEXT): sith.$(LOGOEXT)
	$(CONVERT) $^ $(TRANSPARENT) $@
sith-2.$(LOGOEXT): sith.$(LOGOEXT)
...
cleaner: clean
	$(RM) $(LOGO) $(LOGO_VISIBLE) $(LOGO_MIDVISIBLE) \
	      ...
	      doxygen-sith-logo.$(LOGOEXT)                    \
              gpg-sith-logo.jpg avatar.$(LOGOEXT)             \
	      sphinx-sith-logo.$(LOGOEXT)                     \
	      stackoverflow-sith-logo.$(LOGOEXT)              \
	      small-thumbnail.$(LOGOEXT)                 \
	      large-thumbnail.$(LOGOEXT)                 \
	      wallpaper*.$(LOGOEXT) stripe.jpg           \
	      stripe-icon.$(LOGOEXT) sith-logo.txt            \
	      google-cover-sith-logo.$(LOGOEXT)               \
	      youtube-watermark-sith-logo.$(LOGOEXT)          \
...
clean:
	$(RM) *.dim kali*.$(LOGOEXT) sith*.$(LOGOEXT)         \
	      sith-logo-rot-*.$(LOGOEXT) sith-logo-animated-*.$(LOGOEXT) \
