...
.PRECIOUS:  hermes-rot-*.png
.SECONDARY: hermes-rot-*.png
...
LOGOEXT=png
LOGO=hermes-logo.$(LOGOEXT)
LOGO_VISIBLE=hermes-logo-visible.$(LOGOEXT)
LOGO_MIDVISIBLE=hermes-logo-midvisible.$(LOGOEXT)
ANIMEXT=gif
LOGO_ANIM=hermes-logo-animated.$(ANIMEXT)
LOGO_ANIM_SMALL=hermes-logo-small-animated.$(ANIMEXT)
...
test:
	# extract frames
	convert -coalesce hermes-logo-stego-animated.$(ANIMEXT) hermes-logo-stego-rot-%02d.$(LOGOEXT)
	# de-stego
	for k in `seq -w 0 1 $$(($(NROT) - 1))` ; do \
	  stegosuite -x -f archive.tlrzpq.gpg.part$(D) hermes-logo-stego-rot-%02d.$(LOGOEXT) || exit 2 ; \
	done
	# unsplit
...
#$(LOGO): kali.$(LOGOEXT) hermes-resize.$(LOGOEXT)
$(LOGO): hermes-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(INVISIBLE) $(SHELL) -c '$(GENLOGO)'
#$(LOGO_VISIBLE): kali.$(LOGOEXT) hermes-resize.$(LOGOEXT)
$(LOGO_VISIBLE): hermes-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(VISIBLE) $(SHELL) -c '$(GENLOGO)'
$(LOGO_MIDVISIBLE): hermes-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(MIDVISIBLE) $(SHELL) -c '$(GENLOGO)'

hermes-logo.txt: hermes-logo-visible.$(LOGOEXT)
	img2txt $^ > $@

kali-resize.$(LOGOEXT): kali.$(LOGOEXT) hermes.dim
	$(RESIZE) -resize `cat hermes.dim`^ -extent `cat hermes.dim` $< $@

hermes.dim:
	echo 2000x2000 > $@

#hermes-resize.$(LOGOEXT): hermes-2.$(LOGOEXT) hermes.dim
hermes-resize.$(LOGOEXT): hermes-transparent.$(LOGOEXT) hermes.dim
	$(RESIZE) -resize `cat hermes.dim`^ -extent `cat hermes.dim` $< $@

#hermes-transparent.$(LOGOEXT): hermes.$(LOGOEXT)
#	$(CONVERT) $^ $(TRANSPARENT) $@
hermes-2.$(LOGOEXT): hermes.$(LOGOEXT)
...
kali.jpg: kali.url
	$(WGET)
hermes.png: hermes.url
	$(WGET)



distclean: cleaner
	$(RM) hermes.jpg kali.jpg
...
cleaner: clean
	$(RM) $(LOGO) $(LOGO_VISIBLE) $(LOGO_MIDVISIBLE) \
	      ...
	      doxygen-hermes-logo.$(LOGOEXT)                    \
              gpg-hermes-logo.jpg avatar.$(LOGOEXT)             \
	      sphinx-hermes-logo.$(LOGOEXT)                     \
	      stackoverflow-hermes-logo.$(LOGOEXT)              \
	      small-thumbnail.$(LOGOEXT)                 \
	      large-thumbnail.$(LOGOEXT)                 \
	      wallpaper*.$(LOGOEXT) stripe.jpg           \
	      stripe-icon.$(LOGOEXT) hermes-logo.txt            \
	      google-cover-hermes-logo.$(LOGOEXT)               \
	      youtube-watermark-hermes-logo.$(LOGOEXT)          \
...
clean:
	$(RM) *.dim kali*.$(LOGOEXT) hermes*.$(LOGOEXT)         \
	      hermes-logo-rot-*.$(LOGOEXT) hermes-logo-animated-*.$(LOGOEXT) \
