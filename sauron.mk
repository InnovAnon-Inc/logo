...
.PRECIOUS:  sauron-rot-*.png
.SECONDARY: sauron-rot-*.png
...
LOGOEXT=png
LOGO=sauron-logo.$(LOGOEXT)
LOGO_VISIBLE=sauron-logo-visible.$(LOGOEXT)
LOGO_MIDVISIBLE=sauron-logo-midvisible.$(LOGOEXT)
ANIMEXT=gif
LOGO_ANIM=sauron-logo-animated.$(ANIMEXT)
LOGO_ANIM_SMALL=sauron-logo-small-animated.$(ANIMEXT)
...
test:
	# extract frames
	convert -coalesce sauron-logo-stego-animated.$(ANIMEXT) sauron-logo-stego-rot-%02d.$(LOGOEXT)
	# de-stego
	for k in `seq -w 0 1 $$(($(NROT) - 1))` ; do \
	  stegosuite -x -f archive.tlrzpq.gpg.part$(D) sauron-logo-stego-rot-%02d.$(LOGOEXT) || exit 2 ; \
	done
	# unsplit
...
#$(LOGO): kali.$(LOGOEXT) sauron-resize.$(LOGOEXT)
$(LOGO): sauron-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(INVISIBLE) $(SHELL) -c '$(GENLOGO)'
#$(LOGO_VISIBLE): kali.$(LOGOEXT) sauron-resize.$(LOGOEXT)
$(LOGO_VISIBLE): sauron-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(VISIBLE) $(SHELL) -c '$(GENLOGO)'
$(LOGO_MIDVISIBLE): sauron-resize.$(LOGOEXT) kali-resize.$(LOGOEXT)
	BLEND=$(MIDVISIBLE) $(SHELL) -c '$(GENLOGO)'

sauron-logo.txt: sauron-logo-visible.$(LOGOEXT)
	img2txt $^ > $@

kali-resize.$(LOGOEXT): kali.$(LOGOEXT) sauron.dim
	$(RESIZE) -resize `cat sauron.dim`^ -extent `cat sauron.dim` $< $@

sauron.dim:
	echo 2000x2000 > $@

#sauron-resize.$(LOGOEXT): sauron-2.$(LOGOEXT) sauron.dim
sauron-resize.$(LOGOEXT): sauron-transparent.$(LOGOEXT) sauron.dim
	$(RESIZE) -resize `cat sauron.dim`^ -extent `cat sauron.dim` $< $@

sauron-transparent.$(LOGOEXT): sauron.$(LOGOEXT)
	$(CONVERT) $^ $(TRANSPARENT) $@
#sauron-2.$(LOGOEXT): sauron.$(LOGOEXT)
...
kali.jpg: kali.url
	$(WGET)
sauron.jpg: sauron.url
	$(WGET)



distclean: cleaner
	$(RM) sauron.jpg kali.jpg
...
cleaner: clean
	$(RM) $(LOGO) $(LOGO_VISIBLE) $(LOGO_MIDVISIBLE) \
	      ...
	      doxygen-sauron-logo.$(LOGOEXT)                    \
              gpg-sauron-logo.jpg avatar.$(LOGOEXT)             \
	      sphinx-sauron-logo.$(LOGOEXT)                     \
	      stackoverflow-sauron-logo.$(LOGOEXT)              \
	      small-thumbnail.$(LOGOEXT)                 \
	      large-thumbnail.$(LOGOEXT)                 \
	      wallpaper*.$(LOGOEXT) stripe.jpg           \
	      stripe-icon.$(LOGOEXT) sauron-logo.txt            \
	      google-cover-sauron-logo.$(LOGOEXT)               \
	      youtube-watermark-sauron-logo.$(LOGOEXT)          \
...
clean:
	$(RM) *.dim kali*.$(LOGOEXT) sauron*.$(LOGOEXT)         \
	      sauron-logo-rot-*.$(LOGOEXT) sauron-logo-animated-*.$(LOGOEXT) \
